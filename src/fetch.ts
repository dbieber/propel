...
export function resolve(p: string, isTest?: boolean): URL {
  ...
  if (isTest == null) {
    isTest = (global.PROPEL_TESTER != null && !!global.PROPEL_TESTER);
  }
  ...
  if (isTest && url.hostname === "propelml.org") {
    if (IS_WEB) {
      url.host = document.location.host;
    } else {
      const url2 = new URL("file:///");
      const path = nodeRequire("path");
      url2.pathname = path.join(__dirname, "../build/dev_website/",
                                url.pathname);
      url = url2;
    }
  }
...
async function fetchNodeFS(job: string, url: URL): Promise<ArrayBuffer> {
  ...
  const b = readFileSync(url.pathname, null);
